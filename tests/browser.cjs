...
(async()=>{
 ...
 try{for(const site of ['manapool','scryfall','steamgifts']){
  ...
  assert.equal(await panel.getByText('v2.1.2',{exact:true}).count(),1);
  await panel.getByText('About & diagnostics',{exact:true}).click();assert.match(await panel.locator('#pph-diagnostics').textContent(),/Prism Pride Highlighter 2\.1\.2[\s\S]*Active identities:/);
  ...
  await page.keyboard.press('Escape');assert.equal(await panel.isVisible(),false);await page.keyboard.press('Alt+g');assert.equal(await panel.isVisible(),true);await page.keyboard.press('Alt+g');assert.equal(await panel.isVisible(),false);assert.deepEqual(errors,[]);
