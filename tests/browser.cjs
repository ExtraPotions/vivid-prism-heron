...
(async()=>{
 ...
 try{for(const site of ['manapool','scryfall','steamgifts']){
  ...
  assert.equal(await panel.getByText('v2.1.3',{exact:true}).count(),1);
  await panel.getByText('About & diagnostics',{exact:true}).click();assert.match(await panel.locator('#pph-diagnostics').textContent(),/Prism Pride Highlighter 2\.1\.3[\s\S]*Active identities:/);
